...
const CATEGORY_ROTATION = ["МОДА", "ВОЛОСЫ", "УХОД", "РАЗБОР ТРЕНДА", "ЛУКСМАКСИНГ"];
const EVERGREEN = [
    {
        category: "МОДА",
...
function scoreArticle(article, target, state) {
    const text = `${article.title} ${article.description ?? ""}`.toLowerCase();
    if (EXCLUDED.some((word) => text.includes(word)))
        return -100;
    if (isLikelyDuplicate(article.title, article.url, state.posts))
        return -100;
    ...
    if (category === target)
        score += 9;
    if (article.sourceName && article.sourceName !== "Источник")
        score += 2;
    if (article.title.length >= 35 && article.title.length <= 115)
        score += 2;
    return score;
}
function contextualPoints(category) {
    switch (category) {
        case "МОДА":
            return {
                points: [
...
function buildFromArticle(article) {
...
export async function generatePost(state) {
    ...
    for (const result of settled) {
        if (result.status === "rejected")
            console.warn("Один RSS-источник недоступен:", result.reason);
    }
    ...
    if (ranked[0])
        return buildFromArticle(ranked[0].article);
    if (config.allowEvergreenFallback)
        return fallbackPost(state);
    return {
        skip: true,
        category: "РАЗБОР ТРЕНДА",
        title: "",
