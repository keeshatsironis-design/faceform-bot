...
function charWidthFactor(char) {
    ...
    if (/[.,:;!?'"`|()\[\]{}\-–—]/u.test(char))
        return 0.34;
    if (/[1ilIjtfr]/u.test(char))
        return 0.36;
    if (/[mwMWЖШЩЮФЫжшщюфы@%&]/u.test(char))
        return 0.84;
    if (/[A-ZА-ЯЁ0-9]/u.test(char))
        return 0.68;
    return 0.6;
}
...
function wrapTitle(text, fontSize) {
    ...
    const words = sourceWords.flatMap((word) => estimateTextWidth(word, fontSize) > TITLE_MAX_WIDTH
...
export async function createPostCard(post, channelUsername) {
    const { lines, fontSize, lineHeight } = fitTitle(post.title);
    const titleSvg = lines
        .map((line, index) => `<text x="${TITLE_X}" y="${TITLE_START_Y + index * lineHeight}" class="title">${escapeXml(line)}</text>`)
        .join("\n");
